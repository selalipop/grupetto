...
import com.spop.poverlay.util.livechart.LiveChartAttributes.TAG_PADDING
import com.spop.poverlay.util.livechart.LiveChartAttributes.TAG_WIDTH
...
open class LiveChartView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private var datasetLinePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        ...
        setColor()
        pathEffect = CornerPathEffect(chartStyle.mainCornerRadius)
    ...
    private var secondDatasetPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        ...
        pathEffect = CornerPathEffect(chartStyle.secondCornerRadius)
        strokeCap = Paint.Cap.BUTT
        strokeJoin = Paint.Join.MITER
    }
